  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (connected) {
        ...
        if (window.opener) {
          window.opener.postMessage({ type: "FACEBOOK_CONNECTED", platform: connected }, "*");
      ...
      if (backendError && !code) {
        ...
        if (window.opener) {
          window.opener.postMessage({ type: "FACEBOOK_ERROR", error: backendError }, "*");
      ...
      if (error) {
        ...
        setStatus("error");
        setMessage(errorDescription || "Facebook connection was cancelled");

        if (window.opener) {
          window.opener.postMessage({ type: "FACEBOOK_ERROR", error: "denied" }, "*");
          setTimeout(() => window.close(), 2000);
        }
        return;
      }
      ...
      if (code && state) {
        ...
        if (window.opener) {
          window.opener.postMessage({ type: "FACEBOOK_ERROR", error: "missing_params" }, "*");
          setTimeout(() => window.close(), 2000);
        }
      }
    };
